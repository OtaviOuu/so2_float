defmodule So2FloatWeb.UserAuthTest do
  use So2FloatWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias So2Float.Accounts
  alias So2Float.Accounts.Scope
  alias So2FloatWeb.UserAuth

  import So2Float.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, So2FloatWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      So2FloatWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        So2FloatWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
